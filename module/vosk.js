...
if (os.platform() == 'win32') {
    ...
    let dllDirectory = path.resolve(path.join(__dirname, "..", "node_modules", "vosk", "lib", "win-x86_64"));
    process.env.Path = dllDirectory + path.delimiter + currentPath;

    soname = path.join(__dirname, "..", "node_modules", "vosk", "lib", "win-x86_64", "libvosk.dll")
} else if (os.platform() == 'darwin') {
    soname = path.join(__dirname, "..", "node_modules", "vosk", "lib", "osx-universal", "libvosk.dylib")
} else {
    soname = path.join(__dirname, "..", "node_modules", "vosk", "lib", "linux-x86_64", "libvosk.so")
}
